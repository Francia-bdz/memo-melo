<x-app-layout>
    ...
    <h1 class="text-6xl font-black text-beige text-center mt-10">MON RÉPERTOIRE </h1>
    <h3 class="text-lg text-beige text-center mt-2">Pour se souvenir de chacunes des douces mélodies</h3>

    @if (session()->has('success'))
        <div class="alert alert_success text-beige ml-4 font-bold">
            {{ session()->get('success') }}
        </div>
    @endif

    @if (session()->has('error'))
        <div class="alert alert_error text-beige">
            {{ session()->get('error') }}
        </div>
    @endif
            ...
            @foreach ($mySongs as $song)
                <a href="{{ route('songs.show', $song) }}" title="Voir les informations">
                    <div class="bg-beige w-80 h-52 rounded-3xl border border-black p-10">
                        <h4 class="font-inter text-xl text-green uppercase font-semibold truncate ">{{ $song->title }}
                        </h4>
                        <h5 class="font-inter text-lg text-green/60 truncate">{{ $song->artist }}</h4>
                            <hr class="border-1 border-black/10 my-3">
                            <p class="font-inter font-semibold text-lg text-green">Niveau de connaissance</p>
                            <div class="rate flex gap-1	mt-1">
                                @for ($i = 0; $i < 5; $i++)
                                    @if ($i < $song->globalKnowledge)
