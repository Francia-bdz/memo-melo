    ...
    <div class="bg-beige m-8 p-12 rounded-xl border border-black">
        ...
        <form method="POST" action="{{ route('songs.update', $song) }}" enctype="multipart/form-data">
            ...
            <div class="flex my-8 justify-between">
                ...
                <div class="flex flex-col gap-5">
                    ...
                    <div>
                        ...
                        <x-text-input id="title" value="{{ isset($song->title) ? $song->title : old('title') }}"
                            class="block mt-1 md:min-w-96" type="text" name="title" required autofocus />
                    ...
                    <div>
                        ...
                        <x-text-input id="artist" value="{{ isset($song->artist) ? $song->artist : old('artist') }}"
                            class="block mt-1 md:min-w-96" type="text" name="artist" required autofocus />
                ...
                <div class="flex flex-col gap-5">
                    ...
                    <div>
                        ...
                        <x-text-input id="tabs" value="{{ isset($song->tabs) ? $song->tabs : old('tabs') }}"
                            class="block mt-1 md:min-w-96" type="text" name="tabs" />
                        <x-input-error :messages="$errors->get('tabs')" class="mt-2" />
                    </div>

                    <div>
                        <x-input-label for="link" :value="__('Lien vers la musique')" />
                        <x-text-input id="link" value="{{ isset($song->link) ? $song->link : old('link') }}"
                            class="block mt-1 md:min-w-96" type="text" name="link" />
            ...
            <div class="flex mt-8 justify-between">
                ...
                <div x-data="{ chordsKnowledge: {{ $song->chordsKnowledge }} }" id="chordsKnowledge" class="flex gap-1">
                ...
                <div x-data="{ rythmKnowledge: {{ $song->rythmKnowledge }} }" id="rythmKnowledge" class="flex gap-1">
                ...
                <div x-data="{ globalKnowledge: {{ $song->globalKnowledge }} }" id="globalKnowledge" class="flex gap-1">
...
<div class="flex ">
            <input type="submit" name="valider" value="MODIFIER LES INFORMATIONS DE LA MUSIQUE"
            class=" font-inter bg-beige rounded-full border border-black transition shadow-button hover:bg-beige hover:shadow-buttonHover py-2 px-4 ml-8 mb-6">
    ...
    <form method="POST" action="{{ route('songs.destroy', $song) }}">
        ...
        @method('DELETE')
        <input
            class="'inline-flex items-center py-2 px-4 ml-8 mb-6 text-beige uppercase tracking-widest focus:outline-none focus:ring-2 focus:ring-pink focus:ring-offset-2 bg-red-600 rounded-full border border-black shadow-button hover:bg-red-800 hover:shadow-buttonHover transition ease-in-out duration-150"
            type="submit" value="Supprimer LA MUSIQUE ">
    </form>
